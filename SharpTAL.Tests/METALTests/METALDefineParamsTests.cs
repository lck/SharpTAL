...
namespace SharpTAL.SharpTALTests.METALTests
{
	...
	public class METALDefineParamsTests
	{
		...
		{
			RunTest(@"<html><div metal:define-macro=""one"" class=""funny"" metal:define-param=""string blue null"">Before <i tal:content=""blue"">blue</i> After</div></html>",
				@"<html><body metal:use-macro='site.macros[""one""]'>Nowt <i metal:fill-param='blue ""white""'/> here</body></html>",
		...
		{
			RunTest(@"<html><div metal:define-macro=""one"" class=""funny"">Before <i tal:content=""blue"">blue</i> <i tal:content=""red"">blue</i> After<tal:tag metal:define-param='string blue null;string red ""red""'/></div></html>",
				@"<html><body metal:use-macro='site.macros[""one""]'>Nowt <i metal:fill-param='blue ""white"";red ""orange""'/> here</body></html>",
		...
		{
			RunTest(@"<tal:tag metal:define-macro=""one"" metal:define-param=""int count 0""><i tal:replace=""string:Count=${count};"">100</i><tal:tag tal:define='set count count - 1' tal:condition='count &gt; 0'><tal:tag metal:use-macro='site.macros[""one""]' metal:fill-param='count count'/></tal:tag></tal:tag>",
				@"<body metal:use-macro='site.macros[""one""]'>Nowt <i metal:fill-param='count 5'/> here</body>",
